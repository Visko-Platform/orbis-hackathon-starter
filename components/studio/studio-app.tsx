...
function StudioWorkspace({ clearJwt }: { clearJwt: () => void }) {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      <div className="page-heading"><div><div className="eyebrow">ORBIS AD / {section === "studio" ? "PRODUCT WORKSPACE" : section.toUpperCase()}</div><h1>{section === "studio" ? "Your product, placed live." : section === "campaigns" ? "Brands, ready for their close-up." : section === "library" ? "Set the scene." : section === "architecture" ? "How it works." : "Every direction, in one place."}</h1><p>{section === "studio" ? "Add the product and what is true about it. Then direct the scene as it unfolds." : section === "campaigns" ? "Real artwork from each brand, ready to use in your next placement." : section === "library" ? "Choose a real film clip, then direct what happens next." : section === "architecture" ? "Every model and service in the live path, and what each one does." : "Review and export the creative decisions in this browser."}</p></div><div className="heading-actions">{section !== "architecture" && session.connected && <button className="button subtle" type="button" onClick={() => runAction(session.disconnectSession, "Session disconnected")} disabled={session.busy && !session.runStarted}>Disconnect</button>}{section === "architecture" ? null : section === "activity" ? <button className="button secondary" type="button" disabled={!activity.length} onClick={exportActivity}><Icon name="download" size={16} />Export history</button> : <button className="button secondary" type="button" disabled={locked} onClick={addProductImage}><Icon name="upload" size={16} />Add product image</button>}</div></div>
