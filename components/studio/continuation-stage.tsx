...
export function ContinuationStage({ session, campaign, framePreview, originalPreview, preparing, runId, brandRetained, overlay, knowledgeVersion, contract, onContractChange, onReadContract, canReadContract, onStart, onPivot, onAction, onAddProduct, demoStepId, demoPending, onDemoStep }: Props) {
  ...
  return <div className="preview-column">
    ...
    <section className="contract-panel" aria-labelledby="contract-heading">
      ...
      <form className="contract-add" onSubmit={(event) => { event.preventDefault(); addLine(); }}><input type="text" aria-label="New contract line" value={newLine} maxLength={200} placeholder="Add a line that must stay true, e.g. the man keeps his grey hoodie" disabled={!contract || busy} onChange={(event) => setNewLine(event.target.value)} /><button className="button secondary" type="submit" disabled={!contract || !newLine.trim() || busy}>Add</button></form>
    </section>
  </div>;
}
