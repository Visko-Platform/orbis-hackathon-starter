...
export function AdBreak({ phase, live, onFinished }: Props) {
  ...
  useEffect(() => {
    ...
    const begin = async () => {
      ...
      try {
        ...
        setStatus("live");
        void speak({ campaignId: CAMPAIGN_ID, scene: first.brief, role: "opening", contractLines: contractLines(prepared.contract ?? null) });
      } catch (caught) {
        setStatus("failed");
  ...
  async function runStep(step: { id: string }, source = "") {
    ...
    try {
      ...
      void speak({ campaignId: CAMPAIGN_ID, scene: result.engineered.text, role: result.step.kind === "moment" ? "refine" : "pivot", direction: source || undefined, contractLines: contractLines(result.contract) });
  ...
  async function submitText() {
    ...
    try {
      ...
      void speak({ campaignId: CAMPAIGN_ID, scene: source, role: "pivot", direction: source, contractLines: contractLines(result.contract) });
    } catch (caught) { setError(caught instanceof Error ? caught.message : "That direction did not go through."); }
    finally { setSending(false); }
  }
