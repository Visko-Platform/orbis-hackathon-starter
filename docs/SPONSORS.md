...
The cosmic opening uses one interactive Three.js canvas with continuous camera travel, projected destination buttons, credited NASA imagery mapped into the scene (Webb deep field, cosmic-web simulation, Milky Way concept, SDO Sun, equirectangular Earth) and ISS San Francisco aerial imagery. Multiverse, galaxy and theater geometry and compressed scales are cinematic interpretation. Agency/reference images retain their classifications and credits. A connected model is paused during the opening. Entering the fictional train film starts an anchored live take; custom stories can start from their own prompt without an unrelated forced reference. The opening is not generated scientific footage or an exact physical zoom.
